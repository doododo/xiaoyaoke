    ...
    <ThemeGlobalNext.Provider value={{ searchModal }}>
      <div
        ...
        <main
          ...
          <section
            id='container-inner'
            className={`${siteConfig('XIAOYAOKE_NAV_TYPE', null, CONFIG) !== 'normal' ? 'mt-24' : ''} lg:max-w-3xl xl:max-w-4xl flex-grow md:mt-0 min-h-screen w-full relative z-10`}
            ref={targetRef}>
            {children}
          </section>

          {/* 右侧栏样式 */}
          {siteConfig('XIAOYAOKE_RIGHT_BAR', null, CONFIG) && (
            <SideAreaRight
              targetRef={targetRef}
              slot={rightAreaSlot}
              {...props}
