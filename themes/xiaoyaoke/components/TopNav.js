...
const TopNav = (props) => {
  ...
  // 监听滚动
  useEffect(() => {
    if (siteConfig('XIAOYAOKE_NAV_TYPE', null, CONFIG) === 'autoCollapse') {
      scrollTrigger()
      window.addEventListener('scroll', scrollTrigger)
    }
    return () => {
      siteConfig('XIAOYAOKE_NAV_TYPE', null, CONFIG) === 'autoCollapse' && window.removeEventListener('scroll', scrollTrigger)
    }
  }, [])
  ...
  return (
        <div id='top-nav' className='block lg:hidden'>
            ...
            {/* 导航栏 */}
            <div id='sticky-nav' className={`${siteConfig('XIAOYAOKE_NAV_TYPE', null, CONFIG) !== 'normal' ? 'fixed' : 'relative'} lg:relative w-full top-0 z-20 transform duration-500`}>
